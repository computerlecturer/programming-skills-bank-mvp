    ...
    <Card className="overflow-hidden border-amber-100 bg-gradient-to-br from-amber-50 via-white to-blue-50 shadow-none">
      <CardContent className="px-6 py-12 text-center sm:px-10 sm:py-16">
        ...
        <h2 className="mx-auto max-w-2xl text-3xl font-black tracking-[-0.05em] text-slate-950 sm:text-4xl">
          {subject.label} {questionNo}번 문제는 회원가입 후 이용할 수 있습니다.
        </h2>
        <p className="mx-auto mt-4 max-w-2xl text-base leading-7 text-slate-600">
          비회원도 각 과목 {FREE_LIMIT_PER_SUBJECT}문항까지는 바로 풀 수 있습니다. 현재 베타 서비스 기간에는 회원가입만 하면 전체 620문항, 모의고사, 오답노트, 학습기록 저장 기능을 무료로 이용할 수 있습니다.
        </p>
        <div className="mt-8 grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
            <div className="text-2xl font-black text-slate-950">전체 공개</div>
            <div className="mt-1 text-sm font-bold text-slate-500">베타 회원 혜택</div>
          </div>
          <div className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
            <div className="text-2xl font-black text-slate-950">준비중</div>
            <div className="mt-1 text-sm font-bold text-slate-500">AI 문제 변형</div>
          </div>
        </div>
        <div className="mt-9 flex flex-col justify-center gap-3 sm:flex-row">
          ...
          <ButtonLink href={`/practice/${subject.id}?q=${Math.min(questionNo - 1, FREE_LIMIT_PER_SUBJECT)}`} size="xl" variant="ghost">무료 문제로 돌아가기</ButtonLink>
        </div>
      </CardContent>
    </Card>
  );
